...
func (d *Dino) Show() {
	...
	d.sprite.Show(GfxBankDinos, marvlib.API.MapBanksGet(MapBankDinos).GetArea(MapBankDinoAnimsArea))
...
func Start() {
	marvlib.API.SpritesGet(SpriteBG1).ChangePos(image.Rect(0, 0, 320, 200))
	marvlib.API.SpritesGet(SpriteBG1).Show(GfxBankBG, marvlib.API.MapBanksGet(MapBankDinos).GetArea(MapBankBGArea))
	marvlib.API.SpritesGet(SpriteBG2).ChangePos(image.Rect(320, 0, 320, 200))
	marvlib.API.SpritesGet(SpriteBG2).Show(GfxBankBG, marvlib.API.MapBanksGet(MapBankDinos).GetArea(MapBankBGArea))

	for i := SpriteStart; i <= SpriteEnd; i++ {
		colour := rand.Intn(4)
		Dinos = append(Dinos, NewDino(marvlib.API.SpritesGet(i), randomArenaStartPos(colour), colour))
	}
...
func Update() {
	marvlib.API.SpritesGet(SpriteBG1).ChangePos(image.Rect((bgX / bgScrollScale), 0, 320, 200))
	marvlib.API.SpritesGet(SpriteBG2).ChangePos(image.Rect((bgX/bgScrollScale)+320, 0, 320, 200))
	bgX--
